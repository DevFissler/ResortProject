...
<%@page import="java.util.Date"%>
 ...
 <body>
     <% //JSP문 시작
        ...
        String rootid2=request.getParameter("rootid2");
        
        String c= "";
        
        if(rLevel == 1) {
        	ResultSet rset = stmt.executeQuery("select count(*) from bulletin where rootid2 = "+rootid2+";");
            	
            while (rset.next()) {
            	c = Integer.toString(rset.getInt(1));
            }
            
        } else {
			
        	ResultSet rset = stmt.executeQuery("select count(*) from bulletin where rootid2 like "+rootid2+" ;");
        		
            while (rset.next()) {
            	c = Integer.toString(rset.getInt(1)+1);
            }
        }
        ...
        String QueryTxt2="";
        QueryTxt2=String.format("insert into bulletin (title, reg_date, context,rootid,viewcount,relevel,recount,rootid2) values ('"+title+"','"+temp3+"','"+sCon+"',"+(int)Double.parseDouble(d)+",0,"+rLevel+","+d+","+rootid2+");");
